...
export function TestimonialBlock({ testimonial }: TestimonialBlockProps) {
  return (
    <div className="flex flex-col h-full">
      ...
      {/* min-h keeps all blocks the same height across viewports so neighboring */}
      {/* videos line up regardless of how the identity/stat text wraps. */}
      <div className="mb-4 rounded-lg bg-[#2367EE]/10 border-2 border-[#2367EE]/25 px-4 py-3 md:px-5 md:py-4 min-h-[140px] md:min-h-0 lg:min-h-[170px] flex">
        <div className="flex flex-col md:flex-row md:items-center lg:flex-col lg:items-start gap-3 md:gap-5 lg:gap-3 w-full">
          <div className="flex-1 min-w-0 min-h-[48px] md:min-h-0 lg:w-full">
            <p className="text-[10px] md:text-xs font-bold text-[#2367EE] mb-0.5 tracking-wider uppercase">
              Testimonial #{testimonial.number}
            </p>
            <p className="text-sm md:text-[15px] lg:text-sm text-[#1D4871] leading-snug">
              <span className="font-bold">{testimonial.name}</span>
              <span className="text-[#1D4871]/60"> - {testimonial.attribution}</span>
            </p>
          </div>

          <div className="hidden md:block lg:hidden h-12 w-0.5 bg-[#2367EE]/25 flex-shrink-0" />

          <div className="flex items-center gap-2 md:gap-3 flex-shrink-0 text-[#2367EE] min-h-[44px]">
            <p className="font-comic text-3xl md:text-4xl lg:text-3xl leading-none tracking-wide">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4 md:gap-5 lg:gap-4">
